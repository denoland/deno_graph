...
pub struct Dependency {
  ...
  #[serde(skip_serializing_if = "is_false")]
  pub is_dynamic: bool,
  /// List of imports which refer to this dependency. Doesn't include those
  /// which only supplement types to the main import (i.e. @deno-types).
  #[serde(skip_serializing_if = "Vec::is_empty")]
              ...
              for import in &dep.imports {
                if follow_dynamic || !import.is_dynamic {
                  if let Some(error) = import.errors.first() {
                    self
                      .next_errors
                      .push(ModuleGraphError::ImportError(error.clone()));
                  }
                }
              }
            }
...
pub(crate) fn parse_esm_module_from_module_info(
    ...
    match reference {
      TypeScriptReference::Path(specifier) => {
        ...
        if dep.maybe_type.is_none() {
          dep.maybe_type = resolve(range.clone(), maybe_resolver);
        }
      ...
      TypeScriptReference::Types(specifier) => {
        ...
        let dep_resolution = resolve(range.clone(), maybe_resolver);
  ...
  if matches!(media_type, MediaType::Jsx | MediaType::Tsx) {
    ...
    if let Some(import_source) = res {
      ...
      let range = Range::from_position_range(
        module.specifier.clone(),
        specifier_text.clone(),
        import_source.range,
      );
      if dep.maybe_code.is_none() {
        dep.maybe_code = resolve(range.clone(), maybe_resolver);
      }
      dep.imports.push(Import {
...
impl<'a, 'graph> Builder<'a, 'graph> {
  ...
  async fn fill(
    ...
    for (specifier, slot) in &self.graph.module_slots {
      match slot {
        ModuleSlot::Module(Module::Esm(module)) => module_metadata.insert(
          specifier.clone(),
          ModuleMetadata {
            specifier: module.specifier.clone(),
        ...
        ModuleSlot::Module(Module::Json(module)) => module_metadata.insert(
          specifier.clone(),
          ModuleMetadata {
            specifier: module.specifier.clone(),
            media_type: MediaType::Json,
          ...
          Module::Esm(module) => {
            ...
            for dep in module.dependencies.values_mut() {
              ...
              for import in &mut dep.imports {
                import.errors =
                  check_import_assertions(import, module_metadata);
              }
            }
          }
