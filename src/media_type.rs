...
mod tests {
  ...
  fn resolve_url(url_str: &str) -> Result<ModuleSpecifier> {
    Url::parse(url_str).map_err(|_| anyhow!("Invalid URL"))
  }
  ...
  fn test_from_specifier() {
    ...
    for (specifier, expected) in fixtures {
      let actual = resolve_url_or_path(specifier).unwrap();
  ...
  fn test_from_content_type() {
    ...
    for (specifier, content_type, expected) in fixtures {
      let fixture = resolve_url_or_path(specifier).unwrap();
      assert_eq!(
